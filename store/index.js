  ...
  async getWeather({ commit }, location) {
    try {
      const data = await this.$axios.$get(
        `https://cors-anywhere.herokuapp.com/http://api.openweathermap.org/data/2.5/weather?lat=${location.latitude}&lon=${location.longitude}&appid=${process.env.NUXT_ENV_API_KEY}&units=metric`
      )
